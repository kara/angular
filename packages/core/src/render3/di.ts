...
import {addToViewTree, assertPreviousIsParent, createLContainer, createLNodeObject, getDirectiveInstance, getPreviousOrParentNode, getRenderer, isComponent, renderEmbeddedTemplate, resolveDirective, createTView} from './instructions';
...
export function getOrCreateTemplateRef<T>(di: LInjector): viewEngine_TemplateRef<T> {
  ...
  const tNode = di.node.tNode !;
  const hostTView = di.node.view.tView;
  let templateTView: TView = tNode.data as TView;
  if (templateTView == null) {
    templateTView = tNode.data = createTView(hostTView.directiveRegistry, hostTView.pipeRegistry);
  }
  ngDevMode && assertNotNull(templateTView, 'TView must be allocated');
  return di.templateRef ||
      (di.templateRef = new TemplateRef<any>(
           getOrCreateElementRef(di), templateTView, hostNode.data.template !,
