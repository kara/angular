...
import {allocHostVars, bind, directiveInject, element, elementEnd, elementProperty, elementStart, elementStyling, elementStyleProp, elementStylingApply, listener, load, text, textBinding, loadQueryList, registerContentQuery} from '../../src/render3/instructions';
=======
...
>>>>>>> 020f51e3f... fix(ivy): proper accounting of host vars in case of inherited Directives
...
describe('host bindings', () => {
  ...
  it('should support host bindings with the same name as inputs', () => {
    ...
    class HostBindingInputDir {
      ...
      static ngDirectiveDef = defineDirective({
        ...
        hostBindings: (rf: RenderFlags, ctx: HostBindingInputDir, elIndex: number) => {
          ...
          if (rf & RenderFlags.Update) {
            elementProperty(elIndex, 'disabled', bind(ctx._disabled), null, true);
          }
        },
    ...
    hostBindingInputDir._disabled = true;
    fixture.update();
    expect(hostBindingInputDir.disabled).toBe(false);
